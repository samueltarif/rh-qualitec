export default defineEventHandler(async (event) => {
  try {
    ...
    const existing = await $fetch<any[]>(
      `${supabaseUrl}/rest/v1/configuracoes_comunicacao?empresa_id=eq.${empresa[0].id}&select=id`,
      { headers }
    )
    ...
    if (existing && existing.length > 0) {
      ...
      result = await $fetch<any[]>(
        `${supabaseUrl}/rest/v1/configuracoes_comunicacao?id=eq.${existing[0].id}`,
        {
          method: 'PATCH',
          headers,
          body: {
      ...
      // Criar
      result = await $fetch<any[]>(
        `${supabaseUrl}/rest/v1/configuracoes_comunicacao`,
        {
          method: 'POST',
          headers,
          body: {
    ...
    console.error('[PUT COMUNICACAO] Erro:', error.message || error)
    throw createError({ 
      statusCode: error.statusCode || 500, 
      message: error.message || 'Erro ao salvar configurações de comunicação' 
    })
  }
})
