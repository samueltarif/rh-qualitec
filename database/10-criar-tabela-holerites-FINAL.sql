-- ============================================================================
-- SCRIPT FINAL: Criar tabela de holerites
-- ============================================================================
-- Versão corrigida que funciona com INTEGER IDs
-- ============================================================================
...
DROP POLICY IF EXISTS "Usuarios podem ver seus holerites" ON holerites;
...
-- IMPORTANTE: Como a tabela funcionarios usa INTEGER e não tem vínculo direto com auth.uid(),
-- vamos usar uma abordagem baseada no email_login do funcionário

-- Política: Usuários podem ver seus próprios holerites (baseado no email)
CREATE POLICY "Usuarios podem ver seus holerites"
  ON holerites
  FOR SELECT
  USING (
    EXISTS (
      SELECT 1 FROM funcionarios
      WHERE funcionarios.id = holerites.funcionario_id
      AND funcionarios.email_login = current_setting('request.jwt.claims', true)::json->>'email'
    )
  );
