-- ============================================================================
-- CONSOLIDAÇÃO FINAL - LUCAS E SISTEMA COMPLETO (CORRIGIDO)
-- ============================================================================
...
DECLARE
    ...
    tabela_empresas TEXT;
BEGIN
    -- Verificar qual tabela usar (empresas ou empresa)
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'empresas') THEN
        tabela_empresas := 'empresas';
        EXECUTE 'SELECT COUNT(*) FROM empresas' INTO empresa_count;
        RAISE NOTICE 'Usando tabela empresas: % registros', empresa_count;
    ELSIF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'empresa') THEN
        tabela_empresas := 'empresa';
        EXECUTE 'SELECT COUNT(*) FROM empresa' INTO empresa_count;
        RAISE NOTICE 'Usando tabela empresa: % registros', empresa_count;
    ELSE
        RAISE NOTICE '⚠️  Nenhuma tabela de empresa encontrada!';
        RETURN;
    END IF;
    
    IF empresa_count = 0 THEN
        RAISE NOTICE '⚠️  Nenhuma empresa encontrada. Execute FIX_EMPRESA_CONSTRAINT_DEFINITIVO.sql primeiro';
    ELSE
        -- Pegar ID da primeira empresa
        EXECUTE 'SELECT id FROM ' || tabela_empresas || ' ORDER BY created_at LIMIT 1' INTO empresa_id_padrao;
        ...
        -- Garantir que todos os app_users tenham empresa_id (se coluna existir)
        IF EXISTS (
            ...
            SET empresa_id = empresa_id_padrao
            WHERE empresa_id IS NULL;
            RAISE NOTICE '✅ App_users atualizados com empresa_id';
        ELSE
            RAISE NOTICE 'ℹ️  Coluna empresa_id não existe em app_users';
        END IF;
...
BEGIN
    ...
    -- Contar empresas (verificar qual tabela usar)
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'empresas') THEN
